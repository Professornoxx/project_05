...
async function loadDepositAnalysis() {
  ...
  try {
    ...
    document.querySelector('#channelTable tbody').innerHTML = (d.byChannel || []).map((r) => {
      ...
      return '<tr><td>' + r.channel + '</td><td class="num">' + fmtNum(r.total_orders) + '</td><td class="num">' + fmtNum(r.comp_orders) +
        '</td><td class="num">' + fmtNum(r.comp_users) + '</td><td class="num">' + fmtInr(r.comp_amount) +
        '</td><td class="num ' + successClass(pct) + '">' + pct.toFixed(1) + '%</td><td class="num">' + fmtMin(r.avg_mins) + '</td></tr>';
